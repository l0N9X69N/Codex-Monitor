import { spawnSync } from 'node:child_process';
import process from 'node:process';

const steps = [
  ['Syntax', process.execPath, ['./scripts/check-syntax.mjs']],
  ['Phase 09 manager regression', process.execPath, ['--test',
    'test/unit/phase9-dashboard.test.js',
    'test/unit/phase9-tui.test.js',
    'test/unit/phase9-timeline.test.js'
  ]],
  ['Phase 10 selected analytics', process.execPath, ['--test',
    'test/unit/phase10-session-analytics.test.js'
  ]]
];

for (const [label, command, args] of steps) {
  process.stdout.write(`\n== ${label} ==\n`);
  const result = spawnSync(command, args, { stdio: 'inherit', env: process.env });
  if (result.status !== 0) {
    process.stderr.write(`\nPhase 10 verification FAILED at: ${label}\n`);
    process.exit(result.status ?? 1);
  }
}

process.stdout.write('\nPhase 10 automated verification: PASS\n');
process.stdout.write('Selected-session context/token/turn/tool analytics, live incremental tailing and responsive inspect views are gated; real-session visual acceptance remains pending.\n');
