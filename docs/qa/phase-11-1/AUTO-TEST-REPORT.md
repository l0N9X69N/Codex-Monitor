...
Status: **FINAL CANDIDATE — local verification required**
...
Final recorded PASS must be written only after the current branch is pulled and `npm run verify:phase11-1` succeeds on the target machine.
